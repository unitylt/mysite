...
from django.contrib import auth  # 验证账号密码
from django.contrib.auth.models import User
# forms负责处理所有的验证
class LoginForm(forms.Form):
    username = forms.CharField(label='用户名',  # 创建字段
                               widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': '请输入用户名'}))
    password = forms.CharField(label='密码',  # 括号内自定义内容,密码显示
    ...
    def clean(self):
        ...
        # 这里不能使用request 所以不能处理登录 登录写在总views中
